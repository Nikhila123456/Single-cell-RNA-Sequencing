...
## Seurat Clustering -  Myeloid  cell population
...
# mk <- FindAllMarkers(sample, min.pct = 0.3, min.diff.pct = 0.2, only.pos = T)
# outname = paste0(name, "_mk.rds")
# saveRDS(mk, file = outname)
...
First, we estimate size factor to remove any outliers. Size factors help us normalize for differences in mRNA recovered across cells, and "dispersion" values 
will help us perform differential expression analysis later.   


Before we get into the details of ordering cells along a trajectory, it's important to understand what Monocle is doing. The ordering workflow has three main steps, 
each of which involve a significant machine learning task.
...
```{r brng_to_Monocle, warning=FALSE, message=FALSE, echo=FALSE, tidy=TRUE, results = "hide", fig.height=8, fig.width=12,  eval = F}

setwd("/depot/pccr/data/Low_lab_projects/hr03128_single-cell-RNAseq-study_2021/5_Analysis_with_Monocle2/Myeloid_6Clust")

cds = as.CellDataSet(sample)
...
deg <- readRDS('Myeloid_for_TJ_mk.rds')
